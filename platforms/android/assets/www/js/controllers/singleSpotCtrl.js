...
myApp.controller('SingleSpotCtrl', function($scope,$state,$ionicLoading,$ionicModal,$ionicPopup,$cordovaBarcodeScanner,$timeout,$ionicSideMenuDelegate,$compile,localStorageService,SpotService,LocationService,AWSS3Service,$rootScope,$ionicSlideBoxDelegate,$cordovaFacebook) {
	...
	$scope.onShare = function() {
		...
		facebookConnectPlugin.getLoginStatus(
			function(success) {
				var postImg = $scope.spot.SceneImageDataList[0];
	...
	$scope.onPayToRevealSecret = function(state){
		...
		var playSceneInit = function(){
		  ...
		  var spot = [
		  						...
		  						$scope.spot.Name, 
		  						$scope.spot.Puzzle, 
		  						$scope.spot.Puzzle_Hint, 
		  						$scope.spot.Puzzle_Answer, 
		  						$scope.user.AWS_ACCESS_KEY_ID, 
		  						$scope.user.AWS_SECRET_ACCESS_KEY, 
		  						$scope.spot.Bucket_name
		  					 ];
	...
	$scope.getSceneImage = function() {
		...
		AWSS3Service.getSceneImagesList(AWS_BUCKET_NAME, prefix).then(function(data) {			
      $scope.spot.SceneImageDataList = data;
      console.log("SceneImageDataList", data);
      $timeout(function() {
		...
		.catch(function(err) {
			console.log('Could not load images from S3');
			$scope.sceneImageClass_loading = false;
      $scope.sceneImageClass_noImage = true;
		});	
		/*
